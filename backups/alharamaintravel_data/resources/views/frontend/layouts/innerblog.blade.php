...
<head>
    ...
    <link rel="stylesheet" href="{{ url('/frontend/assets/css/reviews.css') }}">
...
$('#load-more').click(function() {
    ...
    $.ajax({
        url: "/load-more-blogs?page="+ page,
        type: "GET",
        success: function(data) {
            if (data.trim() === '') {
                $('#load-more').hide();
            } else {
                $('#blogsData').append(data);
...
$('#load-more-testimonials').click(function() {
    let page = $(this).data('page');
    $.ajax({
        url: "/load-more-reviews?page="+ page,
        type: "GET",
        success: function(data) {
            if (data.trim() === '') {
                ...
                $('#blogsData').append(data);
                $('#load-more-testimonials').data('page', page + 1);
            }
        }
    });
});
