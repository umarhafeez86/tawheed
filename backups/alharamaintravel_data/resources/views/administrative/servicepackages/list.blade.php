					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/servicepackages/create') }}'">Add New</button>
                                @endif
								</div>
...
@php
$main_services = Services::where('services_id',$servicepackage->services_id)->first();
@endphp
                          <td>
							<strong style="color: #000">{{ $servicepackage->servicepackages_name }}</strong><br>
							<span style="color: #ff0000;">
								@if ($main_services)
									{{ $main_services->services_name }}
								@endif
							</span><br>
							@if ($servicepackage->soldout=="Yes") <span class="label label-danger" id="stock_{{ $servicepackage->servicepackages_id }}" style="cursor: pointer" onclick="updatestockinfo('{{ $servicepackage->servicepackages_id }}');">Sold</span> @else <span class="label label-success" id="stock_{{ $servicepackage->servicepackages_id }}" style="cursor: pointer" onclick="updatestockinfo('{{ $servicepackage->servicepackages_id }}');">Available</span> @endif
							@if ($servicepackage->featured_package!=1) <span class="label label-info" id="featured_{{ $servicepackage->servicepackages_id }}" style="cursor: pointer" onclick="updatefeaturedinfo('{{ $servicepackage->servicepackages_id }}');">Normal</span> @else <span class="label label-success" id="featured_{{ $servicepackage->servicepackages_id }}" style="cursor: pointer" onclick="updatefeaturedinfo('{{ $servicepackage->servicepackages_id }}');">Featured</span> @endif
						</td>
@php
...
@endphp
						  <td>
							<strong>Makkah:</strong>  @if ($makkah_hotel_info){{ $makkah_hotel_info->hotelinfos_name }} @endif <br/>
							<strong>Madinah:</strong> @if ($madinah_hotel_info){{ $madinah_hotel_info->hotelinfos_name }} @endif
						  </td>
						  
						  <td>{{ $servicepackage->servicepackages_price }}</td>
						  <td>{{ $servicepackage->servicepackages_sort }}</td>

						  <td>@if ($servicepackage->servicepackages_status == 1) <span class="label label-success">Enable</span> @else <span class="label label-danger">Disable</span> @endif</td>

						  <td>@if ($servicepackage->showall=="Yes") <span class="label label-success" id="showall_{{ $servicepackage->servicepackages_id }}" style="cursor: pointer" onclick="updateshowallinfo('{{ $servicepackage->servicepackages_id }}');">Yes</span> @else <span class="label label-danger" id="showall_{{ $servicepackage->servicepackages_id }}" style="cursor: pointer" onclick="updateshowallinfo('{{ $servicepackage->servicepackages_id }}');">No</span> @endif</td>
                          ...
                          <td>
							...
							@if (userpermission_status(session()->get('adminusergroups_id'),"17_e")==true)
                            <a href="{{ route("administrative.servicepackages.edit",$servicepackage->servicepackages_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"17_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $servicepackage->servicepackages_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $servicepackage->servicepackages_id }}" action="{{ route("administrative.servicepackages.destroy",$servicepackage->servicepackages_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
