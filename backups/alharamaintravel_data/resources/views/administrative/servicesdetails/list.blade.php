				...
				<div class="row heading-bg">
					...
					<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
						<ol class="breadcrumb">
							...
							<li><a href="{{ url('administrative/services/index') }}">Manage Services</a></li>
@php
					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/servicesdetails/create') }}/{{ $services_id }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
						  <td>{{ $servicesdetail->servicesdetails_id }}</td>
						  <td>{{ $servicesdetail->servicesdetails_heading }}</td>
                          <!--td>{!! strlen($servicesdetail->servicesdetails_details) > 100 ? substr($servicesdetail->servicesdetails_details, 0, 100) . '...' : $servicesdetail->servicesdetails_details !!}</td-->
                          <td>{{ $servicesdetail->servicesdetails_sort }}</td>
                          <td>@if ($servicesdetail->servicesdetails_status ==1) Enabled @else Disabled @endif</td>
                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"6_e")==true)
                            <a href="{{ route("administrative.servicesdetails.edit",$servicesdetail->servicesdetails_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"6_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $servicesdetail->servicesdetails_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $servicesdetail->servicesdetails_id }}" action="{{ route("administrative.servicesdetails.destroy",$servicesdetail->servicesdetails_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
