					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/emailtemplates/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          ...
                          <td>{{ $emailtemplate->email_title }}</td>
                          <td>{{ $emailtemplate->email_subject }}</td>
						  
						  <td>
							@if ($emailtemplate->status ==1) Enabled @else Disabled @endif
						  </td>

                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"16_e")==true)
                            <a href="{{ route("administrative.emailtemplates.edit",$emailtemplate->id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"16_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $emailtemplate->id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $emailtemplate->id }}" action="{{ route("administrative.emailtemplates.destroy",$emailtemplate->id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
