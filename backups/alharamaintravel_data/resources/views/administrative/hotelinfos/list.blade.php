					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/hotelinfos/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $hotelinfo->hotelinfos_id }}</td>
                          <td><strong style="color: #000">{{ $hotelinfo->hotelinfos_name }}</strong></td>

						  <td>{{ $hotelinfo->hotelinfos_city }}</td> 
						  <td>{{ $hotelinfo->hotelinfos_startype }} Star</td> 

						  <td>{{ $hotelinfo->hotelinfos_sort }}</td>

						  <td>@if ($hotelinfo->hotelinfos_status == 1) <span class="label label-success">Enable</span> @else <span class="label label-danger">Disable</span> @endif</td>

                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"18_e")==true)
                            <a href="{{ route("administrative.hotelinfos.imagesgallery",$hotelinfo->hotelinfos_id) }}" style="padding:0 5px;">
							...
							@if (userpermission_status(session()->get('adminusergroups_id'),"18_e")==true)
                            <a href="{{ route("administrative.hotelinfos.edit",$hotelinfo->hotelinfos_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"18_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $hotelinfo->hotelinfos_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $hotelinfo->hotelinfos_id }}" action="{{ route("administrative.hotelinfos.destroy",$hotelinfo->hotelinfos_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
