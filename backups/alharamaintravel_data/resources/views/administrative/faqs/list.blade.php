					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/faqs/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $faq->faqs_id }}</td>
                          <td>{{ $faq->faqs_heading }}</td>
                          
                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"7_e")==true)
                            <a href="{{ route("administrative.faqs.edit",$faq->faqs_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"7_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $faq->faqs_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $faq->faqs_id }}" action="{{ route("administrative.faqs.destroy",$faq->faqs_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
