					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/packages/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
						  ...
						  <td>{{ $package->packages_price }}</td>
						  
						  <td>
							@if ($package->packages_status ==1) Enabled @else Disabled @endif
						  </td>

						  <td>{{ $package->packages_sort }}</td>

                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"9_e")==true)
                            <a href="{{ route("administrative.packages.edit",$package->packages_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"9_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $package->packages_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $package->packages_id }}" action="{{ route("administrative.packages.destroy",$package->packages_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
