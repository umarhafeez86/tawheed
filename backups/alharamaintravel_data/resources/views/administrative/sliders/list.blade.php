					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/sliders/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $slider->sliders_id }}</td>
                          <td>{{ $slider->sliders_heading }}</td>
						  
						  <td>
							@if ($slider->sliders_status ==1) Enabled @else Disabled @endif
						  </td>
                          ...
                          <td>
                            ...
                            <a href="{{ route("administrative.sliders.edit",$slider->sliders_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"18_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $slider->sliders_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $slider->sliders_id }}" action="{{ route("administrative.sliders.destroy",$slider->sliders_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
