					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/usergroups/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $usergroup->usergroups_id }}</td>
                          <td>{{ $usergroup->usergroups_name }}</td>
                          <td>{{ get_formated_date($usergroup->created_at,"d M, Y") }}</td>
                          <td>@if ($usergroup->usergroups_status == 1) <span class="label label-success">Enable</span> @else <span class="label label-danger">Disable</span> @endif</td>

                          <td>
                            
							@if ($usergroup->usergroups_id > 0)
								
							@if (userpermission_status(session()->get('adminusergroups_id'),"3_e")==true)
                            <a href="{{ route("administrative.usergroups.edit",$usergroup->usergroups_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"3_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $usergroup->usergroups_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $usergroup->usergroups_id }}" action="{{ route("administrative.usergroups.destroy",$usergroup->usergroups_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
