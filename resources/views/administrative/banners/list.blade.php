					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/banners/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $banner->banners_id }}</td>
                          <td>{{ $banner->banners_name }}</td>
						  
						  <td>
								@if ($banner->banners_for == 0) Umrah @elseif($banner->banners_for == 1) Umrah & Holidays @elseif($banner->banners_for == 2) Featured Holidays @elseif($banner->banners_for == 3) Text Block @endif
						  </td>

						  <td>
							@if ($banner->banners_status ==1) Enabled @else Disabled @endif
						  </td>

						  <td>{{ $banner->banners_sort }}</td>

                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"13_e")==true)
                            <a href="{{ route("administrative.banners.edit",$banner->banners_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"13_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $banner->banners_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $banner->banners_id }}" action="{{ route("administrative.banners.destroy",$banner->banners_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
