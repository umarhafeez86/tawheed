					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/partners/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $partner->partners_id }}</td>
                          <td>{{ $partner->partners_name }}</td>
                          
						  <td>
							@if ($partner->partners_status ==1) Enabled @else Disabled @endif
						  </td>

						  <td>
							@if ($partner->partners_image != "")
							<img height="50" src="{{ asset('uploads/partners/'.$partner->partners_image) }}">
							@endif
						  </td>
                          ...
                          <td>
                            ...
                            <a href="{{ route("administrative.partners.edit",$partner->partners_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"14_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $partner->partners_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $partner->partners_id }}" action="{{ route("administrative.partners.destroy",$partner->partners_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
