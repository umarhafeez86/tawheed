...
<div class="clearfix"></div>
              ...
              <form data-toggle="validator" novalidate="true" action="{{ route("administrative.faqs.update",$faqs->faqs_id) }}" method="post" enctype="multipart/form-data" >
                @method('put')
                @csrf
                <div class="form-group">
                  <label class="control-label mb-10 text-left">Heading</label>
                  <input type="text" name="faqs_heading" class="form-control @error("faqs_heading") is-invalid @enderror" data-error="Value Required." value="{{ old("faqs_heading",$faqs->faqs_heading) }}" required >
          ...
          <div class="form-group col-xs-12">
                  ...
                  <div class="clearfix"></div>
                  @if ($faqs->faqs_image != "")
                  <img height="50" src="{{ asset('uploads/faqs/'.$faqs->faqs_image) }}">
                  @endif
          </div> 
              ...
              <input type="file" name="faqs_image">
                ...
                <div class="form-group" id="faqslist">
                  @php
                      ...
                      $faqsdetails = Faqsdetails::where('faqs_id',$faqs->faqs_id)->orderBy('faqsdetails_sort','asc')->get();
                  @endphp
                  <div class="form-group">
                    <input type="hidden" name="rownumber" id="rownumber" value="{{ $faqsdetails->count() }}">
                    <a id="add-more1" name="add-more" class="btn btn-primary" style="padding: 4px 13px;" onclick="faqslist_fields();">+ Add Faqs List</a>
                  </div>
                  @php
                  $rownumber = 1;	
                  @endphp
                  @foreach ($faqsdetails as $faqsdetail)
                  <div class="form-group" id="faqslistrow{{ $rownumber }}">
                      <div class="form-group">
                          ...
                          <input type="text" name="faqsdetails_question[]" class="form-control" value="{{ $faqsdetail->faqsdetails_question }}">
                      </div>
                      <div class="form-group">
                          <label class="control-label text-left">Answer</label>
                          <textarea name="faqsdetails_answer[]" class="textarea_editor form-control" rows="5">{{ $faqsdetail->faqsdetails_answer }}</textarea>
                      </div>
                      <div class="form-group">
                          <label class="control-label text-left">Sort</label>
                          <input type="text" name="faqsdetails_sort[]" class="form-control" value="{{ $faqsdetail->faqsdetails_sort }}">
                      </div>
                      <div class="form-group">
                          <label class="control-label mb-10 text-left">Status</label>
                          <select name="status[]" class="form-control">
                            <option value="1" @if ($faqsdetail->status==1) selected @endif>Enabled</option>
                            <option value="0" @if ($faqsdetail->status==0) selected @endif>Disabled</option>
                          </select>
                      </div>
				          </div>
                  @php
