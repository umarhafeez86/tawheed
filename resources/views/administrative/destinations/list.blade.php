					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/destinations/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $destination->destinations_id }}</td>
                          <td>{{ $destination->destinations_name }}</td>
						  <td>
							@if ($destination->destinations_status ==1) Enabled @else Disabled @endif
						  </td>
						  <td>{{ $destination->destinations_sort }}</td>
                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"20_e")==true)
                            <a href="{{ route("administrative.destinations.edit",$destination->destinations_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"20_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $destination->destinations_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $destination->destinations_id }}" action="{{ route("administrative.destinations.destroy",$destination->destinations_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
