					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/airlineslogos/create') }}'">Add New</button>
                                @endif
								</div>
									...
									<div class="table-wrap">
										<div class="table-responsive">
											<table id="datable_1" class="table  display table-hover mb-30">
												...
												<tbody>
                        @foreach ($airlineslogos as $destination)
                        <tr>
                          <td>{{ $destination->airlineslogos_id }}</td>
                          <td>{{ $destination->airlineslogos_name }}</td>
						  <td>
							@if ($destination->airlineslogos_status ==1) Enabled @else Disabled @endif
						  </td>
						  <td>{{ $destination->airlineslogos_sort }}</td>
                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"22_e")==true)
                            <a href="{{ route("administrative.airlineslogos.edit",$destination->airlineslogos_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"22_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $destination->airlineslogos_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $destination->airlineslogos_id }}" action="{{ route("administrative.airlineslogos.destroy",$destination->airlineslogos_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
