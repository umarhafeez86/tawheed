					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/testimonials/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $testimonial->testimonials_id }}</td>
                          <td>{{ $testimonial->testimonials_name }}</td>
                          
						  <td>
							{{ $testimonial->testimonials_location }}
						  </td>

						  <td>
							@if ($testimonial->status ==1) Enabled @else Disabled @endif
						  </td>
						  
						  <td>{{ $testimonial->testimonials_sort }}</td>

                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"12_e")==true)
                            <a href="{{ route("administrative.testimonials.edit",$testimonial->testimonials_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"12_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $testimonial->testimonials_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $testimonial->testimonials_id }}" action="{{ route("administrative.testimonials.destroy",$testimonial->testimonials_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
