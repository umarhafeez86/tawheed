					...
					<div class="col-sm-12">
						<div class="panel panel-default card-view">
							<div class="panel-heading">
                                ...
                                <button class="btn  btn-primary" onclick="window.location='{{ url('administrative/blogs/create') }}'">Add New</button>
                                @endif
								</div>
                        ...
                        <tr>
                          <td>{{ $blog->blogs_id }}</td>
                          <td>{{ $blog->blogs_name }}</td>
                          
						  <td>{{ $blog->blogs_date }}</td>
						  <td>{{ $blog->blogs_author }}</td>

						  <td>
							@if ($blog->blogs_status ==1) Enabled @else Disabled @endif
						  </td>

                          <td>
                            
							@if (userpermission_status(session()->get('adminusergroups_id'),"15_e")==true)
                            <a href="{{ route("administrative.blogs.edit",$blog->blogs_id) }}" style="padding:0 5px;">
                              <i class="fa fa-edit" aria-hidden="true"></i>
                            </a>
                            @endif

							@if (userpermission_status(session()->get('adminusergroups_id'),"15_d")==true)
                            <a href="javascript:void(0)" onclick="deleteData({{ $blog->blogs_id }});">
								<i class="fa fa-trash" aria-hidden="true"></i>
							</a>
                            <form id="delete-data-from-{{ $blog->blogs_id }}" action="{{ route("administrative.blogs.destroy",$blog->blogs_id) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form> 
							@endif
