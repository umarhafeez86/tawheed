...
<head>
	...
	<!-- Morris Charts CSS -->
    <link href="{{ url('admin/vendors/bower_components/morris.js/morris.css') }}" rel="stylesheet" type="text/css"/>
    ...
    <link rel="stylesheet" href="{{ url('admin/vendors/bower_components/bootstrap3-wysihtml5-bower/dist/bootstrap3-wysihtml5.css') }}" />
	...
	<!-- Select2 CSS -->
	<link href="{{ url('admin/dist/css/select2.min.css') }}" rel="stylesheet" type="text/css">
</head>
...
<body>
    ...
    <script src="{{ url('admin/vendors/bower_components/wysihtml5x/dist/wysihtml5x.min.js') }}"></script>
    
    <script src="{{ url('admin/vendors/bower_components/bootstrap3-wysihtml5-bower/dist/bootstrap3-wysihtml5.all.js') }}"></script>

	<!-- Bootstrap Wysuhtml5 Init JavaScript -->
    <script src="{{ url('admin/dist/js/bootstrap-wysuhtml5-data.js') }}"></script>

	<!-- Data table JavaScript -->
	<script src="{{ url('admin/vendors/bower_components/datatables/media/js/jquery.dataTables.min.js') }}"></script>
	<script src="{{ url('admin/dist/js/dataTables-data.js') }}"></script>
	...
	<!-- select2 JavaScript -->
	<script src="{{ url('admin/dist/js/select2.min.js') }}"></script>
...
function faqslist_fields() { 
	$rownumber = parseInt($("#rownumber").val()) + 1;
	$("#rownumber").val($rownumber);
	$.ajax
	({ 
		url: "{{ url('administrative/faqs/addnewrow') }}"+"/"+$rownumber,
		success: function(result)
		{
			$("#faqslist").append(result);
		}
	});	
	
}

function remove_faqslist_fields(rid) {
	$('#faqslistrow'+rid).remove();
}


function serviceslist_fields() { 
	$rownumber = parseInt($("#rownumber").val()) + 1;
	$("#rownumber").val($rownumber);
	$.ajax
	({ 
		url: "{{ url('administrative/services/addnewrow') }}"+"/"+$rownumber,
		success: function(result)
		{
			$("#serviceslist").append(result);
		}
	});	
	
}

function remove_serviceslist_fields(rid) {
	$('#serviceslistrow'+rid).remove();
}
