...
class MainServicesController extends Controller
{
    ...
    public function holidaypackageshome()
    {
        $staticpages = Staticpages::findorFail(2);
        return view("frontend.indexholidays", [
            'page_title'     => $staticpages->pages_title,
    ...
    public function holidaypackages($destinationpackages_url)
    {
                    $destination = Destinations::where("destinations_url",$destinationpackages_url)->first();

                    $services_title     = $destination->destinations_title;
                    $services_name      = $destination->destinations_name;
                    $services_descp     = $destination->destinations_descp;
                    $services_keyword   = $destination->destinations_keyword;
                    $services_details   = $destination->destinations_details;
                    $services_image     = $destination->destinations_image2;

                    $servicepackages = Servicepackages::where('servicepackages_status', 1)
                                ->where('destinations_id', $destination->destinations_id)
                                ->orderBy('servicepackages_sort', 'asc')
                                ->paginate(6); // Initial load


                    return view('frontend.services.holidaypackages',[
                        'page_title'      => $services_title,
                        'page_name'       => $services_name,
                        'page_descp'      => $services_descp,
                        "page_keyword"    => $services_keyword,
    ...
    public function loadMoreHolidaysPackages(Request $request)
    {
        if ($request->ajax()) {
                $servicepackages = Servicepackages::where('servicepackages_status', 1)
                    ->where('destinations_id', $request->destinations_id)
                    ->orderBy('servicepackages_sort', 'asc')
                    ->paginate(6, ['*'], 'page', $request->page);
            
            return view('frontend.services.showmoreholidaypackages', compact('servicepackages'))->render();
        }
    }
